function para_local = run_optimal_MSE(para_init,Npara,lb,ub,setcase)
    tic
    % 4. Objective function
    fitnessFcn = @(param) fitness_MSE(param,setcase);
    initial_fitness = fitness_MSE(para_init,setcase);
    nonlcon = @(param) fitness_constraint_MSE(param, initial_fitness,setcase);
    ...
    fprintf('Initial fitness=%.6f | Optimized fitness=%.6f\n', fitness_MSE(para_init,setcase), fitness_MSE(para_local,setcase));
    for i = 1:Npara
    ...
    outputFile = fullfile(generated_results_dir(), 'bl_cfd_final_optimized_parameters_MSE.mat');
