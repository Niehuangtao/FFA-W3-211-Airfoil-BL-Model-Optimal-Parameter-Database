function [CC_total,CN_total,CM_total,CD,CL,CM,a] = cal_DS(param,conditions,airfoildata)
    ...
    %%%%%%%%%%%%%%%%%%%%Initialize lag states and variables%%%%%%%%%%%%%%%%%%%%
    %---------------------1.Attached flow----------------------
    ...
    D_Mq = 0; 
    CN_cir   = zeros(n_steps,1);  % circulatory normal-force coefficient
    CN_a_cir = zeros(n_steps,1);  % circulatory angle-of-attack contribution
    CN_q_cir = zeros(n_steps,1);  % circulatory pitch-rate contribution
    CN_imp   = zeros(n_steps,1);  % impulsive normal-force coefficient
    CN_imp_a = zeros(n_steps,1);  % impulsive angle-of-attack contribution
    CN_imp_q = zeros(n_steps,1);  % impulsive pitch-rate contribution
    CN_pot   = zeros(n_steps,1);  % attached-flow normal-force coefficient
    CC_pot   = zeros(n_steps,1);  % attached-flow chordwise-force coefficient
    ...
    alpha_E  = zeros(n_steps,1);  % 3/4chord angle of attack
    %---------------------2.Separated flow----------------------
     
    Df = 0;
    alpha_f= zeros(n_steps,1);
    CN_lag = zeros(n_steps,1);  % lagged normal-force coefficient
    f_orig = zeros(n_steps,1);  % steady separation-point location
    f      = zeros(n_steps,1);  % unsteady separation-point location
    f_lag  = zeros(n_steps,1);  % unsteady separation-point location
    CN_f   = zeros(n_steps,1);  % separated-flow normal-force coefficient
    CC_f   = zeros(n_steps,1);  % separated-flow chordwise-force coefficient
    CM_f   = zeros(n_steps,1); 
    %---------------------3.Vortex shedding----------------------
    tau_v = zeros(n_steps,1);   % dimensionless leading-edge vortex position
    VRTX  = zeros(n_steps,1);   % vortex shedding flag
    TESF  = zeros(n_steps,1);
    LESF  = zeros(n_steps,1);
    target= zeros(n_steps,1);
    Cv    = zeros(n_steps,1);               
    CC_V  = zeros(n_steps,1);   % vortex-induced chordwise-force coefficient
    CN_V  = zeros(n_steps,1);   % vortex-induced normal-force coefficient
    ...
    %%%%%%%%%%%%%%%%%%%%Advance all components through time%%%%%%%%%%%%%%%%%%%%
    for i = 1 : n_steps
        if i == 1
            %------------------------1.Attached flow-------------------------
            ...
            Cv(i) = CN_cir(i)*(1-((1+sqrt(f_lag(i)))/2)^2);
            if LESF(i) == 1
                if tau_v(i) < Tvl && (Cv(i)-Cv(i-1))*(a(i)) > 0
