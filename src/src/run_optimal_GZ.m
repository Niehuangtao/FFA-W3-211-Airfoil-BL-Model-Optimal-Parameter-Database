function para_local = run_optimal_GZ(para_init,Npara,lb,ub,setcase)
    tic
    % 4. Objective function
    fitnessFcn = @(param) fitness_GZ(param,setcase);
    initial_fitness = fitness_GZ(para_init,setcase);
    nonlcon = @(param) fitness_constraint_GZ(param, initial_fitness,setcase);
    ...
    fprintf('Initial fitness=%.6f | Optimized fitness=%.6f\n', fitness_GZ(para_init,setcase), fitness_GZ(para_local,setcase));
    for i = 1:Npara
    ...
    outputFile = fullfile(generated_results_dir(), 'bl_cfd_final_optimized_parameters_GZ.mat');
